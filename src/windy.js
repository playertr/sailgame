...
var Windy = function( params ){
    var MAX_PARTICLE_NUM = 8000;
    var VELOCITY_SCALE = 0.005;             // scale for wind velocity (completely arbitrary--this value looks nice)
    var MAX_WIND_INTENSITY = 40;              // wind velocity at which particle intensity is maximum (m/s)
    var MAX_PARTICLE_AGE = 500;                // max number of frames a particle is drawn before regeneration
    ...
    var PARTICLE_REDUCTION = 1.0;            // reduce particle count to this much of normal for mobile devices
    ...
    var animate = function(bounds, field) {
      ...
      var geo_area = Math.abs((bounds.north - bounds.south)) * Math.abs((bounds.west - bounds.east))
      var geo_mult = Math.pow(geo_area, 0.5)

      geo_mult=1
  
      console.log("Geo area is: ")
      console.log(geo_area)
      console.log("Geo mult is: ")
      console.log(geo_mult)

      var particleCount = Math.round(geo_mult * bounds.width * bounds.height * PARTICLE_MULTIPLIER);
      if (isMobile()) {
        particleCount *= PARTICLE_REDUCTION;
      }
      particleCount = Math.min(particleCount, MAX_PARTICLE_NUM)
